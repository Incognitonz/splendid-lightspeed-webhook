  ...
  try {
    ...
    const labServiceProductIds = {
      fast: [
        ...
        '1f21c3ff-f618-443c-a2af-5bc0cc26b9c9', // SKU 11994 - other film formats 220 - FAST
        // SPL400
        '31562d7d-0818-4c28-92a5-af769511592c', // SKU 12119 - spl400 social - FAST
        '328f8d4f-8baa-4f99-af34-0df5db7f55a5', // SKU 12122 - spl400 medium - FAST
        // Noritsu
      ...
      '3day': [
        ...
        '6b58abf4-e0cf-4f84-b8d9-5e9d9c3213eb', // SKU 12699 - prepaid bw social - 3 Day
        'a5c982db-ef80-4b7c-8b37-8f6d208610bf', // SKU 12702 - prepaid bw medium - 3 Day
        // SPL400
        'ac8cd51c-97ad-463f-b4c0-11432838a7e4', // SKU 12118 - spl400 social - 3 Day
        'bc694495-6785-48da-9370-0aeb9528087b', // SKU 12121 - spl400 medium - 3 Day
        // Noritsu
    ...
    // NZ Public Holidays API configuration
    ...
    // Function to fetch NZ public holidays for a given year
    const fetchPublicHolidays = async (year) => {
      try {
        ...
        const response = await fetch(`${HOLIDAYS_API_URL}?key=${HOLIDAYS_API_KEY}&year=${year}&countryCode=NZ`);
        
        if (!response.ok) {
          console.log(`Failed to fetch holidays for ${year}: ${response.status}`);
          return [];
        }
    ...
    // Function to check if a date is a public holiday (for C41 processing - you're open weekends but closed holidays)
    const isOperatingDay = async (timestamp) => {
      const date = new Date(timestamp);
      
      // Check if it's a public holiday (you're closed on holidays)
      const isHoliday = await isPublicHoliday(date);
      return !isHoliday;
    };
    ...
    // Function to move a date to the next operating day (for C41 - excludes only holidays, not weekends)
    ...
    // Function to move a date to the next business day (for B&W - excludes weekends and holidays)
    ...
    const calculateDueDate = async (turnaroundType, isBwFilm = false) => {
      ...
      switch (turnaroundType) {
        case 'fast':
          ...
          // For fast service: B&W moves to next business day, C41 moves to next operating day (excludes only holidays)
      ...
      for (const lineItem of data.line_items) {
        ...
        if (isLabService(productId)) {
          ...
          if (!alreadyHasDueDate) {
            ...
            // Calculate due date (now async to handle public holidays)
